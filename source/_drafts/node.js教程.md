...
### buffer的相关方法
+ 创建指定大小的buffer：```var buf = Buffer.alloc(10) //创建一个10字节的buffer```。会在内存中分配一个长度为10的连续空间。 还有个```var buf = Buffer.allocUnSafe(10)```，也是创建一个指定大小为10字节的buffer，但是buffer中可能含有敏感数据。也就是alloc分配空间的同时清空数据，但allocUnSafe只分配，不清空数据。
  > alloc：allocate，分配。
