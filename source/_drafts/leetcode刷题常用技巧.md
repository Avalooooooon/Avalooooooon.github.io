...
2. 方法```str.split(" ")```：把字符串分割成字符串数组。可以传一个参数作为切割标志，不传则默认切割每一个字符。
3. 方法```xxx.toString()```：常用于将数值转换为对应进制的数字字符串。
...
#### Map
> 关键词：key个数不确定；除了知道是否出现，还需要知道出现位置等其他信息（存在value里）；哈希值比较少、特别分散、跨度非常大（此时使用数组作为哈希表会产生很大浪费）；
> 特殊：无限**循环**（快乐数）
> 缺点： 占用空间比数组大，速度比数组慢
