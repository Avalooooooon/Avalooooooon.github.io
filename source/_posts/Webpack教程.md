---
title: Webpack教程
updated: 2023-03-09 14:25:12
date: 2022-10-13 12:56:02
tags: [Webpack]
categories: [前端,Webpack]
---

> Webpack是JavaScript应用程序的静态资源打包工具。
> webpack处理应用程序时会递归地构建一个依赖关系图，以一个或多个文件作为打包的入口，将整个项目所有文件编译组合成一个或多个文件输出出去。
> “打包”，就是编译，把浏览器不识别的语法编译成可以识别的语法。输出的文件就是编译好的文件，就可以在浏览器端运行了。
> 我们将这种 Webpack 输出的文件叫做 bundle。
> <span style='color:red'>相较于无差别打包依赖模块的传统打包器，webpack的核心优势在于它从入口文件出发，递归构建依赖关系图。通过这样的依赖梳理，webpack打包出的bundle不会包含重复或未使用的模块，实现了按需打包，极大的减少了冗余。</span>
[Webpack在线课件](https://yk2012.github.io/sgg_webpack5/base)

## Webpack作用

浏览器仅能识别html,css和js，而无法识别开发时我们会使用的框架（React、Vue），ES6 模块化语法，Less/Sass 等 css 预处理器等语法。这样的代码要想运行，必须经过编译，编译成浏览器能识别的 JS、Css 等语法，才能运行。 
而webpack则可以将这些文件解析打包成浏览器能识别的基本文件。编译成浏览器可以识别的语法，最终才能在浏览器中运行。
打包工具还能压缩代码、做兼容性处理、提升代码性能等。
比如说，模块化开发中，我们会编写大量模块，如果不打包就上线，那么页面加载或交互时，将会发起大量资源请求。为了性能优化，需要使用webpack这样的打包器对模块进行打包整合，以减少请求数。<span style='color:red'>就像简单的vue项目，所有组件最终都将被打包到一个app.js中。</span>

除了最常用的Webpack外，比较常见的打包工具还有Grunt、Gulp、Rollup、Vite等。
