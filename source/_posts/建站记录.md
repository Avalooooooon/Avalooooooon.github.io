...
+ ```git push origin hexo```将本次修改推送到远端hexo；
...
+ 此时通过命令```git g```、```git s```即可在本地开启同步下来的博客静态页面，并通过 http://localhost:4000/ 访问。
...
+ ```hexo d -g ```将博文静态页面发布至maser分支。hexo本地文件夹中的public文件夹存放的是根据.md生成的html文件，也就是博客的静态文件。执行```hexo d```就是把public文件夹下的文件同步到github。

### 2 创建新分支
我们之前在GitHub创建好的仓库默认只有main分支，这个分支存放的是静态文件。所以，可以在该仓库另外创建一个hexo分支以存放Hexo网站文件，且因为我们只需要手动管理Hexo网站文件，故可以把hexo分支设置为默认分支。
![新分支](hexobranch.png)
